...
export default class RoadTripsPage extends Component {
    ...
    getData(){
        axios.get('http://localhost:8000/user/roadtrips/' + localStorage.getItem('UserId'),{}).then((response)=>{
            ...
            const contents = data.reduce((acc, item, i) => {
                rowContents.push(<Card key={i} id={item._id} title={item.title} budget={item.budget} actions="edit" dates={item.dates}
